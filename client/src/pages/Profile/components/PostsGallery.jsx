...
const PostGallery = (props) => {
  const { myPosts: myPosts, savedPosts: savedPosts } = props.profilePosts;
  const [renderPostList, setRenderPostList] = useState(myPosts);

  const changeTabHandler = (event) => {
    document.querySelector('.post-tab-active').classList.remove('post-tab-active');
  ...
  return (
    <div className="mt-5">
      <div className="d-flex flex-row text-white gap-5 fs-4 fw-light border-bottom border-white">
        <p className="post-tab-active" onClick={changeTabHandler}>My posts</p>
        <p className="" onClick={changeTabHandler}>Saved posts</p>
      </div>
      <div className="posts-container overflow-auto">{renderPostList.map((post, index) => {
