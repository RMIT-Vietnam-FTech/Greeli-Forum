	...
	async function handleData() {
		try {
			...
			if (addedTopics.length <= 0) {
				...
				const loadingNewThreadPopup = toast.loading(
					"Createring new thread is in progress",
				);
				...
				toast.dismiss(loadingNewThreadPopup);
				{
					res.data &&
						toast.success("new thread is created successfully", {
							duration: 3000,
						});
				}
				navigate(
					`/forum/communities/${belongToThread}/posts/${res.data}`,
				);
			}
		} catch (e) {
	...
	return (
		<div>
			...
			<p className="text-secondary">
				Add atleast 1 and maximum 3 topics to help people find your
				thread
			</p>
			<div
...
function AddedButton({ topicName, topicId, addedTopics, setAddedTopics }) {
	function handleAddBtn(topicObject) {
		const topicIndex = addedTopics.findIndex(
