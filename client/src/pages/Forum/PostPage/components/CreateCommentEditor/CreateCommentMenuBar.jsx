...
export default function CreateCommentMenuBar() {
  const { editor } = useCurrentEditor();
  const { file, setFile } = useContext(CommentContext);
  ...
  return (
    <>
      <div className="mb-3">
        ...
        <ImageTag setFile={setFile}  file={file} setDropFile={setDropFile} editor={editor}>
          <CiImageOn />
        </ImageTag>
        <VideoTag setFile={setFile} setDropFile={setDropFile} editor={editor}>
          <MdOndemandVideo />
        </VideoTag>
      </div>
      {dropFile && (
        <div
          ...
          onMouseEnter={() => {
            const trashBinBtn = document.querySelector("#trash-bin-btn");
        ...
        >
          {dropFile.type === "image" ? (
            ...
            <ReactPlayer className="w-100" controls url={dropFile.src} />
          )}
          <button
            id="trash-bin-btn"
