  ...
  return (
    ...
    >
      ...
      <p className="text-white w-100 d-flex justify-content-between">
        <b className="w-50">Threads</b> <span className="ms-3 w-50">{data.thread}</span>
      </p>
      <p className="text-white w-100 d-flex justify-content-between">
        <b className="w-50">Posts</b> <span className="ms-3 w-50">{data.post}</span>
      </p>
      <p className="text-white w-100 d-flex justify-content-between">
        <b className="w-50">Members</b> <span className="ms-3 w-50">{data.user}</span>
      </p>
      </div>
    </div>
  );
...
function ForumLeaderBoard() {
  ...
  return (
    <div
      className="mt-4 w-100 bg-primary-green d-flex flex-column gap-2 align-items-start overflow-scroll-y h-75"
      style={{ borderRadius: "20px" }}
    >
      {data.map((leadeboard, index) => {
        return (
          ...
          >
            <div className="ms-2 d-flex gap-2 align-items-center justify-content-between" style={{width:"30%"}}>
              <Avatar src={leadeboard.profileImage} />
              <div>{index + 1}</div>
            </div>
            <div style={{ fontSize: "12px", width:"60%" }}>{leadeboard.username}</div>
          </div>
        );
      })}
    </div>
